...
public class Game {
    ...
    public boolean doChecks() {
        ...
        char winnerMessage = checkGameWinner(grid);
        if (winnerMessage != '-') {
            gui.gameOver(winnerMessage + " Wins!");
            newGame(false);
            return true;
        }
        return false;
     ...
     */
    public char checkGameWinner(char [][]grid){
        char result = '-';
        ...
        for (int i=0; i<3; i++){
            columnWinner[i] = grid[i][0];
            for (int j=1; j<3; j++){
                if (grid[i][j] != columnWinner[i]) {
                    // no winner in the column
                    columnWinner[i] = '-';
                    break;
                }
            }
        ...
        // Check for a row win
        for (int j=0; j<3; j++){
            rowWinner[j] = grid[0][j];
            for (int i=1; i<3; i++){
                if (grid[i][j] != rowWinner[j]) {
                    // no winner in the row
                    rowWinner[j] = '-';
                    break;
                }
            }
        ...
        // Diaginal wins
        diagnalWinner[0] = grid[0][0];
        for (int i=1; i<3; i++){
            if (grid[i][i] != diagnalWinner[0]) {
                // no winner in the diagnal
                diagnalWinner[0] = '-';
                break;
            }
        }

        diagnalWinner[1] = grid[0][2];
        for (int i=1; i<3; i++){
            if (grid[i][3 - (i+1)] != diagnalWinner[1]) {
                // no winner in the diagnal
                diagnalWinner[1] = '-';
                break;
            }
        ...
        // if a row or column won, return the winner
        for (int xx = 0; xx<3; xx++) {
            if (columnWinner[xx] != '-') {
                return columnWinner[xx];
            } else if (rowWinner[xx] != '-') {
                return rowWinner[xx];
            }
        }
